...
body { padding-top: 1em; padding-bottom: 1em;}    
  ...
  <body>
    <div class="container">
        <h2>The auction is over! Thank you for participating in the ${auction.name}</h2>
        <#if (wonItems?size > 0)>
        <h4><hr>
            <p>${auction.endOfAuctionInstructions!""}</p>
            <hr>
            <p>Please contact ${auction.organizer} at ${auction.organizerEmail} if you have any questions.</p>
            <hr>
        </h4>
        
        <h4><strong>Total Donation: ${totalDonation?string.currency!"0.00"}</strong></h4>
        <hr>
        <h4>Congratulations! You have the winning bid on the following items</h4>
        <table class="table table-condensed">
        <tr><th>Item</th><th>Description</th><th>Donor</th><th>Bid Amount</th></tr>
        <#list wonItems as item>
        <tr><td>${item.name}</td><td>${item.description}</td><td>${item.donor}</td><td>${(item.getHighBidForUser(user))?string.currency!"$0.00"}</td></tr>
        </#list>
        </table>
        </#if>
        <#if (lostItems?size > 0)>
        <h4>Sorry! You were outbid on these items</h4>
        <table class="table table-condensed">
        <tr><th>Item</th><th>My Bid</th><th>Outbid By</th></tr>
        <#list lostItems as item>
        <tr><td>${item.name}</td><td>${(item.getHighBidForUser(user))?string.currency!"$0.00"}</td><td>${(item.getAmountOutBidByForUser(user))?string.currency!"$0.00"}</td></tr>
        </#list>
        </table>
        </#if>
     </div>
